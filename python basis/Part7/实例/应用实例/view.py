...
class InputFrame(Frame):
    ...
    def createPage(self):
        ...
        def openfile():
            text.delete(1.0,END)
            csvreader=csv.reader(open('python\python basis\Part7\文件\招生人数.csv','r',encoding="utf-8"))
        ...
        Button(self,text="保存数据",width=10,command=savefile).pack(side='right',padx=10)
...
class AddFrame(Frame):
    ...
    def add_num(self):
        ...
        if self.S1.get():
            ...
            info.append('无')
        with open('python\python basis\Part7\文件\招生人数.csv','a',newline='',encoding="utf-8")as t:
            write=csv.writer(t)
            write.writerow(info)
    ...
    def createPage(self):
        ...
        Button(self, text='添加',command=self.add_num).grid(row=6, column=1, sticky=E,pady=10)
...
class DeleteFrame(Frame):
    ...
    def del_num(self):
        if self.S1.get()=='':
            ms.showinfo(title='出错',message='输入年份不能为空')
        if self.E1.get()=='':
            ms.showinfo(title='出错',message='输入专业不能为空')
        if self.comb.get()=='':
            ms.showinfo(title='出错',message='输入民族不能为空')
        csvreader=csv.reader(open('python\python basis\Part7\文件\招生人数.csv','r',encoding="utf-8"))
        final_list=list(csvreader)
        for i in range(0,len(final_list)):
            if final_list[i][0] ==self.S1.get() and final_list[i][1] == self.E1.get() and final_list[i][2]==self.comb.get():
                delnum=i
    ...
    def createPage(self):
        ...
        self.comb=ttk.Combobox(self,value='汉族、蒙古族、回族、藏族、满族、维吾尔族、土家族、哈萨克族'.split('、'))
        self.comb.grid(row=3, column=1, sticky=E)
        Button(self,text='删除',command=self.del_num).grid(row=6,column=1,sticky=E,pady=10)